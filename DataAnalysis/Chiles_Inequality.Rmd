...
long_country_names = data_frame(country=c("Macedonia, former Yugoslav Republic of","Serbia and Montenegro","West Bank and Gaza","Hong Kong (China)","Korea, Republic of","Dominican Republic"),
...
country_colours_1 <- data_frame(country=countries_1,colour=as.character(colours_1))
...
country_colours_2 <- data_frame(country=countries_2,colour=as.character(colours_2))
...
income_colours <- data_frame(incomegroup2=income_list,colour=as.character(colours_3))
...
decile_colours <-data_frame(decile=deciles,colour=as.character(colours_4))
...
To start, let's look at the most known measurement for inequality, the [Gini Coefficient](https://en.wikipedia.org/wiki/Gini_coefficient). The below chart is the result of  plotting this number against the GDPP per capita of several countries including <font color="`r colours_1[which("Chile" == countries_1)]`">Chile</font>:
...
plot_data <- comparison_base %>% filter(year_key %in% year_selector_1$year_key) %>% select(country,year,incomegroup,year_key,gini_reported,gdp_ppp_pc_usd2011) %>% group_by(country,year,incomegroup,year_key) %>% summarise(gini=mean(gini_reported),gdp_ppp_pc=mean(gdp_ppp_pc_usd2011)) %>% ungroup() %>% mutate(incomegroup2=ifelse(country=="Chile","Chile",incomegroup)) %>% merge(x=.,y=income_colours,by="incomegroup2",all.x=TRUE)   %>% arrange(-gdp_ppp_pc) 

#for future reference
countries_gini <- plot_data$country

plot_data  %>% hchart("scatter", hcaes(y = gini, x = gdp_ppp_pc,color=colour)) %>% hc_tooltip(useHTML = TRUE, headerFormat = "", pointFormat = tltip) %>%  hc_exporting( enabled = TRUE) %>%  hc_xAxis(title = list(text = "Gini Index"),opposite = FALSE,labels = list(rotation=-90)) %>% hc_yAxis(title = list(text = "GDP Per Capita"), opposite = FALSE, minorTickInterval = "auto", minorGridLineDashStyle = "LongDashDotDot",showFirstLabel = FALSE,showLastLabel = FALSE, plotBands = list(list(from = 25, to = JS("Infinity"), color = "rgba(100, 0, 0, 0.1)"))) %>% hc_title(text = "GDP per capita and Gini Coefficient", margin = 20, align = "left",style = list(color = "#808080", useHTML = TRUE)) %>%  hc_credits(enabled = TRUE,  text = "Source: World Income Inequality Dabase - UNU-WIDER",href = "https://www.wider.unu.edu/project/wiid-world-income-inequality-database") 
...
#filter countries around D10

plot_data <- comparison_dec  %>% filter(year_key %in% year_selector_1$year_key) %>% mutate(incomegroup2=ifelse(country=="Chile","Chile",incomegroup)) %>% merge(x=.,y=income_colours,by="incomegroup2",all.x=TRUE)  %>% select(-incomegroup2) %>% select(key,country,year,decile,incomegroup,colour,gdp_ppp,gdp_ppp_pc_usd2011) %>% group_by(key,country,year,decile,incomegroup,colour) %>% summarise(gdp_ppp=mean(gdp_ppp),gdp_ppp_t=mean(gdp_ppp_pc_usd2011)) %>% ungroup() %>% arrange(-gdp_ppp)

plot_data$index <- seq.int(nrow(plot_data))


chiles_positions_dec <-plot_data  %>% filter(country=="Chile") %>% select(decile,index) %>% arrange(decile) 

plot_data <- plot_data  %>% filter(index %in% seq(chiles_positions_dec$index[index_pos]-countries_around,chiles_positions_dec$index[index_pos]+countries_around))  
  

plot_data   %>% hchart("column", hcaes(y = gdp_ppp, x = key ,color=colour)) %>% hc_tooltip(useHTML = TRUE, headerFormat = "", pointFormat = tltip) %>%  hc_exporting( enabled = TRUE) %>%  hc_xAxis(title = list(text = "Country"),opposite = FALSE,labels = list(rotation=-90)) %>% hc_yAxis(title = list(text = "GDP Per Capita"), opposite = FALSE, minorTickInterval = "auto", minorGridLineDashStyle = "LongDashDotDot",showFirstLabel = FALSE,showLastLabel = FALSE, plotBands = list(list(from = 25, to = JS("Infinity"), color = "rgba(100, 0, 0, 0.1)"))) %>% hc_title(text = "GDP Per Capita - Chile's 10th Decile", margin = 20, align = "left",style = list(color = "#808080", useHTML = TRUE)) %>%  hc_credits(enabled = TRUE,  text = "Source: World Income Inequality Dabase - UNU-WIDER",href = "https://www.wider.unu.edu/project/wiid-world-income-inequality-database") 
...
```{r salary_d1, include=FALSE}

index_pos <- 1
#filter countries around D1
...
plot_data   %>% hchart("column", hcaes(y = gdp_ppp, x = key ,color=colour)) %>% hc_tooltip(useHTML = TRUE, headerFormat = "", pointFormat = tltip) %>%  hc_exporting( enabled = TRUE) %>%  hc_xAxis(title = list(text = "Country"),opposite = FALSE,labels = list(rotation=-90)) %>% hc_yAxis(title = list(text = "GDP Per Capita"), max=max_y,opposite = FALSE, minorTickInterval = "auto", minorGridLineDashStyle = "LongDashDotDot",showFirstLabel = FALSE,showLastLabel = FALSE, plotBands = list(list(from = 25, to = JS("Infinity"), color = "rgba(100, 0, 0, 0.1)"))) %>% hc_title(text = "GDP Per Capita - Chile's 1st Decile", margin = 20, align = "left",style = list(color = "#808080", useHTML = TRUE)) %>%  hc_credits(enabled = TRUE,  text = "Source: World Income Inequality Dabase - UNU-WIDER",href = "https://www.wider.unu.edu/project/wiid-world-income-inequality-database") 
```
...
plot_data %>% hchart("scatter",hcaes(x =country, y = ratio_D10, color = colour)) %>% hc_tooltip(useHTML = TRUE, headerFormat = "", pointFormat = tltip) %>%  hc_exporting( enabled = TRUE) %>%  hc_xAxis(title = list(text = "Country ordered by GDP PPP"),opposite = FALSE,labels = list(rotation=-90)) %>% hc_yAxis(title = list(text = "D10 Ratio (D10/Dx)"), max=max_y,opposite = FALSE, minorTickInterval = "auto", minorGridLineDashStyle = "LongDashDotDot",showFirstLabel = FALSE,showLastLabel = FALSE, plotBands = list(list(from = 25, to = JS("Infinity"), color = "rgba(100, 0, 0, 0.1)"))) %>% hc_title(text = "GDP per capita of highest earning decile  as ratio of other deciles", margin = 20, align = "left",style = list(color = "#808080", useHTML = TRUE)) %>%  hc_credits(enabled = TRUE,  text = "Source: World Income Inequality Dabase - UNU-WIDER",href = "https://www.wider.unu.edu/project/wiid-world-income-inequality-database") 
```
...
x <- c("Country", "Decile", "Deecile's GDP PPP (per capita) ","Overall GDP PPP (per capita)")
y <-c("{point.country}", "{point.decile}", "{point.gdp_ppp:d,.0f}","{point.overall_gdp_ppp:d,.0f}")
tltip <- tooltip_table(x, y)



plot_data <- comparison_dec %>% filter(year_key %in% year_selector_1$year_key)   %>% group_by(country,year,decile,key) %>% summarise( gdp_ppp=mean(gdp_ppp), overall_gdp_ppp=mean(gdp_ppp_pc_usd2011)) %>% merge(x=.,y=country_colours_1,by="country",all.x=TRUE) %>% mutate(colour=ifelse(is.na(colour),other_country_colour,colour)) %>% arrange(-gdp_ppp) 
  
max_values<-plot_data$gdp_ppp
max_y <- max(max_values)+3
  
plot_data %>%  hchart("coloredarea",hcaes(x = key, y = gdp_ppp, segmentColor = colour),zoomType = "x")  %>% hc_tooltip(useHTML = TRUE, headerFormat = "", pointFormat = tltip) %>%  hc_exporting( enabled = TRUE) %>%  hc_xAxis(title = list(text = "Country deciles ordered by GDP PPP"),opposite = FALSE,labels = list(enabled=FALSE)) %>% hc_yAxis(title = list(text = "Per Capita Income"), max=max_y,opposite = FALSE, minorTickInterval = "auto", minorGridLineDashStyle = "LongDashDotDot",showFirstLabel = FALSE,showLastLabel = FALSE, plotBands = list(list(from = 25, to = JS("Infinity"), color = "rgba(100, 0, 0, 0.1)"))) %>% hc_title(text = "Income per decile compared", margin = 20, align = "left",style = list(color = "#808080", useHTML = TRUE)) %>%  hc_credits(enabled = TRUE,  text = "Source: World Income Inequality Dabase - UNU-WIDER",href = "https://www.wider.unu.edu/project/wiid-world-income-inequality-database")  
...
All the above points notwhitstanding, it is important to point that **<font color="`r colours_1[which("Chile" == countries_1)]`">Chile</font> has indeed progressed significantly in the last `r year2-year1` years**. Let's look at the evolution of the Gini coefficient and GDPP per capita, which seems to challenge the perception that growth has brought increased inequality:
...
plot_data <- comparison_base %>% filter(country %in% countries_2) %>% select(country,year,incomegroup,year_key,gini_reported,gdp_ppp_pc_usd2011) %>% group_by(country,year,incomegroup,year_key) %>% summarise(gini=mean(gini_reported),gdp_ppp_pc=mean(gdp_ppp_pc_usd2011)) %>% ungroup()  %>% merge(x=.,y=country_colours_2,by="country") %>% arrange(-gdp_ppp_pc) 

plot_data  %>% hchart("scatter", hcaes(y = gini, x = gdp_ppp_pc,color=colour)) %>% hc_tooltip(useHTML = TRUE, headerFormat = "", pointFormat = tltip) %>%  hc_exporting( enabled = TRUE) %>%  hc_xAxis(title = list(text = "GDP per capita"),opposite = FALSE,labels = list(rotation=-90)) %>% hc_yAxis(title = list(text = "GDP Per Capita"), opposite = FALSE, minorTickInterval = "auto", minorGridLineDashStyle = "LongDashDotDot",showFirstLabel = FALSE,showLastLabel = FALSE, plotBands = list(list(from = 25, to = JS("Infinity"), color = "rgba(100, 0, 0, 0.1)"))) %>% hc_title(text = "Changes GDP per capita and Gini Coefficient", margin = 20, align = "left",style = list(color = "#808080", useHTML = TRUE)) %>%  hc_credits(enabled = TRUE,  text = "Source: World Income Inequality Dabase - UNU-WIDER",href = "https://www.wider.unu.edu/project/wiid-world-income-inequality-database") 
...
plot_data  %>% filter(country==chosen_country, year %in% 1990:2020) %>% hchart("line",hcaes(x =year, y = ratio_D10, group=decile, color = colour)) %>% hc_tooltip(useHTML = TRUE, headerFormat = "", pointFormat = tltip) %>%  hc_exporting( enabled = TRUE) %>%  hc_xAxis(title = list(text = "Year"),opposite = FALSE,labels = list(rotation=-90)) %>% hc_yAxis(title = list(text = "D10 Ratio (D10/Dx)"), opposite = FALSE, minorTickInterval = "auto", minorGridLineDashStyle = "LongDashDotDot",showFirstLabel = FALSE,showLastLabel = FALSE, plotBands = list(list(from = 25, to = JS("Infinity"), color = "rgba(100, 0, 0, 0.1)"))) %>% hc_title(text = paste("D10 Ratio (D10/Dx)",chosen_country,sep=" - "), margin = 20, align = "left",style = list(color = "#808080", useHTML = TRUE)) %>%  hc_credits(enabled = TRUE,  text = "Source: World Income Inequality Dabase - UNU-WIDER",href = "https://www.wider.unu.edu/project/wiid-world-income-inequality-database") 
...
data_plot %>% kable(col.names = c("Decile",                           paste("GDP PPP",year1,sep=" "),
  ...
  footnote(general = "Source: World Income Inequality Dabase - UNU-WIDER") %>%
...
* Refrain the impulse and ditch everything. Looking at the numbers,  it looks like some things are right.
...
list<-ls() 
files<-data_frame(name=append(list,c("list","files")))
